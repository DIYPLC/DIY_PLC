...
void FcTaskCyclic1(void)
{
  ...
  {
    GV.MW[4] = 504; //P = 5 Bar
    GV.MW[5] = 4005; //F = 40 Hz
    GV.MW[6] = 0b100; //AUTO MODE
    GV.Di1 = true;
  }
  ...
  Db_1PIRCA1.HmiSetpoint            = GV.MW[4]                     ; //Заданное значение давления 0...1000 (0...10[Бар]) от HMI.
  Db_1PIRCA1.HmiControlSignalManual = GV.MW[5]                     ; //Сигнал управления в ручном режиме работы 0...5000 (0...50[Гц]) от HMI.
  Db_1PIRCA1.HmiControlWord         = GV.MW[6]                     ; //Слово управления передается от HMI.
  ...
  GV.MW[7]                          = Db_1PIRCA1.HmiProcessVariable; //Измеренное значение регулируемого параметра. 0...1000 (0...10[Бар]) на HMI.
  GV.MW[8]                          = Db_1PIRCA1.HmiControlSignal  ; //Сигнал управления скорости привода 0...5000 (0...50[Гц]) на HMI.
  GV.MW[9]                          = Db_1PIRCA1.HmiStatusWord     ; //Слово состояния на HMI.
  GV.MW[10]                         = Db_1PIRCA1.HmiErrorWord      ; //Слово ошибок на HMI.

  //Карта регистров MODBUS HOLDING REGISTERS  Slave Address 1
  //HMI --> MW[ 4] --> PLC (uint16) HmiSetpoint
  //HMI --> MW[ 5] --> PLC (uint16) HmiControlSignalManual
  //HMI --> MW[ 6] --> PLC (uint16) HmiControlWord
  //HMI <-- MW[ 7] <-- PLC (uint16) HmiProcessVariable
  //HMI <-- MW[ 8] <-- PLC (uint16) HmiControlSignal
  //HMI <-- MW[ 9] <-- PLC (uint16) HmiStatusWord
  //HMI <-- MW[10] <-- PLC (uint16) HmiErrorWord
