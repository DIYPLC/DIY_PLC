...
struct Modbus
{
  ...
  uint16_t CntADU      ; //Адрес последнего элемента для ADU[].
  uint16_t CntADUHEX   ; //Адрес последнего элемента для ADUHEX[].
  uint16_t CntPDU      ; //Адрес последнего элемента для PDU[] (количество байт в массиве - 1).
  uint16_t Counter4    ; //Счетчик переданных байт для ADU[].
  uint8_t  State       ; //Граф состояний.
  uint8_t  UDR         ; //Байт для UART.
  uint8_t  ADU[32]     ; //Патек в формате ASCII. //rx ascii
  uint8_t  ADUHEX[16]  ; //Патек в формате HEX начиная с адреса заканчивая суммой (без : CR LF)
